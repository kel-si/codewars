function getRainfallValues(strng, town) {
  ...
  const statNums = cityStats.match(/[+-]?\d+(\.\d+)?/g);
...
function mean(town, strng) {
  if (strng.includes(`${town}:`)) {
    const numberArr = getRainfallValues(strng, town);
...
function variance(town, strng) {
  if (strng.includes(`${town}:`)) {
    const statMean = mean(town, strng);
    const numArr = getRainfallValues(strng, town);

    let deviance = [];
    for (let stat of numArr) {
      deviance.push(Math.pow(statMean - stat, 2));
    }
    return deviance.reduce((a, b) => a + b, 0) / deviance.length;
  } else {
    return -1;
  }
}
